...
main() {
    ...
    RELEASE_VERSION=$(bin/androidx --version)
    RELEASE_FILE_NAME="androidx-$RELEASE_VERSION.tar.gz"

    run tar zvcf "$RELEASE_FILE_NAME" bin/androidx zsh-completion/_androidx
    ...
    sed_in_place "s|androidx-[0-9].[0-9].[0-9]|androidx-$RELEASE_VERSION|g" README.md
    ...
    run gh release create v"$RELEASE_VERSION" "androidx-$RELEASE_VERSION.tar.gz" "androidx-$RELEASE_VERSION-1-any.pkg.tar.gz" --notes "'release $RELEASE_VERSION'"

    run git clone git@github.com:leleliu008/homebrew-fpliu.git
    run cd homebrew-fpliu
    sed_in_place '/url      /d' Formula/androidx.rb
    sed_in_place '/sha256   /d' Formula/androidx.rb
    sed_in_place "/homepage/a \  sha256   \"$RELEASE_FILE_SHA256SUM\"" Formula/androidx.rb
    sed_in_place "/homepage/a \  url      \"https://github.com/leleliu008/androidx/releases/download/v$RELEASE_VERSION/androidx-$RELEASE_VERSION.tar.gz\"" Formula/androidx.rb
    run git add Formula/androidx.rb
    run git commit -m "'publish new version androidx-$RELEASE_VERSION'"
    run git push origin master
    ...
    run rm "androidx-$RELEASE_VERSION-1-any.pkg.tar.gz"
    run rm -rf pkg
    run rm -rf src
}
